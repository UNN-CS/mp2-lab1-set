...
TBitField::TBitField(int len)
{
	try
	{
		...
		if (reckon == 0) // выделение одного телема
			MemLen = 1;
		else
		{
...
{
	int temp = n / (sizeof(TELEM)* 8);  // нахождение в каком TELEM
	return n - temp*(sizeof(TELEM)*8);  // нахождение индекса в найденном TELEM
...
int TBitField::GetLength(void) const // получить длину (количество битов)
{
	return BitLen;
}
...
int TBitField::GetBit(const int n) const // получение значение бита
{
	try
	{
		if (n < 0) throw 1;
